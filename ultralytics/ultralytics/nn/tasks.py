...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            c1 = ch[f]
            args = [*args[1:]]
        elif m is C2fVSS or m is C2fVSSBlock:
            # C2fVSS 模块：参数格式 [c2, n, shortcut, g, e, d_state, d_conv]
            # 注意：C2fVSS在repeat_modules中，外层的repeats会被插入到args[2]
            c1 = ch[f] if isinstance(f, int) else ch[f[0]] if isinstance(f, (list, tuple)) and len(f) > 0 else ch[f]
            c2 = args[0] if len(args) > 0 else c1
            if c2 != nc:  # 如果不是分类输出
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            # 提取参数：YAML格式是 [c2, n, shortcut, g, e, d_state, d_conv]
            # C2fVSS构造函数需要 [c1, c2, n, shortcut, g, e, d_state, d_conv]
            # 注意：YAML中的n是C2fVSS内部的VSS块数量，不是外层的repeats
            n_internal = args[1] if len(args) > 1 else 1  # C2fVSS内部的n
            shortcut = args[2] if len(args) > 2 else False
            g = args[3] if len(args) > 3 else 1
            e = args[4] if len(args) > 4 else 0.5
            d_state = args[5] if len(args) > 5 else 16
            d_conv = args[6] if len(args) > 6 else 3
            # 构建参数列表：[c1, c2, n_internal, shortcut, g, e, d_state, d_conv]
            args = [c1, c2, n_internal, shortcut, g, e, d_state, d_conv]
            # 注意：外层的repeats（n）不会影响C2fVSS的参数，它只是重复整个C2fVSS模块
        elif m is DCNv4Conv or m is DCNv4Conv2d:
            # DCNv4 模块：参数格式 [c2, k, s, p, g, d, act, deform_groups]
            c1, c2 = ch[f], args[0]
            if len(args) >= 2:
                k = args[1]
            else:
            ...
            if len(args) >= 7:
                ...
                act = True
            if len(args) >= 8:
                deform_groups = args[7]
            else:
                deform_groups = 1
            args = [c1, c2, k, s, p, g, d, act, deform_groups]
        elif m is BiFormerNeck:
            # BiFormer 模块：参数格式 [c2, num_heads, n_win, topk, ...]
            # 需要转换为 [c1, c2, num_heads, n_win, topk, ...]
            c1 = ch[f] if isinstance(f, int) else ch[f[0]] if isinstance(f, (list, tuple)) and len(f) > 0 else ch[f]
            c2 = args[0] if len(args) > 0 else c1
            # 确保通道数被正确设置
            if c2 != nc:  # 如果不是分类输出
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            # 重新组织参数：从 [c2, num_heads, n_win, topk] 到 [c1, c2, num_heads, n_win, topk]
            if len(args) >= 1:
                args = [c1, c2, *args[1:]]  # 插入 c1 作为第一个参数
            else:
                args = [c1, c2]  # 默认值
        elif m is HaarDownsample or m is HaarDownsample2d:
            # Haar小波下采样：输入通道数c1，输出通道数自动为c1*4
            c1 = ch[f] if isinstance(f, int) else ch[f[0]] if isinstance(f, (list, tuple)) and len(f) > 0 else ch[f]
            c2 = c1 * 4  # Haar下采样输出通道数是输入的4倍
            # 如果YAML中指定了输出通道数，用于验证
            if len(args) > 0:
                expected_c2 = args[0]
                if expected_c2 != c2:
                    LOGGER.warning(
                        f"HaarDownsample输出通道数不匹配: "
                        f"输入={c1}, 期望输出={expected_c2}, 实际输出={c2}. "
                        f"将使用实际输出={c2}"
                    )
            # HaarDownsample只需要输入通道数作为参数
            args = [c1]  # 只传入输入通道数
        elif m is SPDConv or m is SPDConv2d:
            # SPD-Conv：空间到深度卷积，用于解决点信息丢失
            c1 = ch[f] if isinstance(f, int) else ch[f[0]] if isinstance(f, (list, tuple)) and len(f) > 0 else ch[f]
            c2 = args[0] if len(args) > 0 else c1
            if c2 != nc:
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            # SPDConv参数: [c2, k, s, p, g, d, act, scale_factor]
            # 但实际构造函数是: (c1, c2, k, s, p, g, d, act, scale_factor)
            if len(args) >= 1:
                args = [c1, c2, *args[1:]]  # 插入c1作为第一个参数
            else:
                args = [c1, c2]
        elif m is DSConv or m is DSConv2d or m is SnakeConv:
            # Dynamic Snake Convolution：动态蛇形卷积，用于处理线条化特征
            c1 = ch[f] if isinstance(f, int) else ch[f[0]] if isinstance(f, (list, tuple)) and len(f) > 0 else ch[f]
            c2 = args[0] if len(args) > 0 else c1
            if c2 != nc:
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            # DSConv参数: [c2, k, s, p, g, d, act, deform_groups]
            # 但实际构造函数是: (c1, c2, k, s, p, g, d, act, deform_groups)
            if len(args) >= 1:
                args = [c1, c2, *args[1:]]  # 插入c1作为第一个参数
            else:
                args = [c1, c2]
        elif m is SEAM or m is SEAMBlock:
            # SEAM模块：堆叠逐元素注意力模块，用于上下文信息提取
            c1 = ch[f] if isinstance(f, int) else ch[f[0]] if isinstance(f, (list, tuple)) and len(f) > 0 else ch[f]
            c2 = args[0] if len(args) > 0 else c1
            if c2 != nc:
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            # SEAM参数: [c2, kernel_sizes, reduction, act]
            # 但实际构造函数是: (c1, c2, kernel_sizes, reduction, act)
            # kernel_sizes 可以是列表或单个值，需要从YAML中正确提取
            if len(args) >= 1:
                # 提取参数：c2已经在上面提取了
                kernel_sizes = args[1] if len(args) > 1 else [3, 5, 7]  # 默认多尺度
                reduction = args[2] if len(args) > 2 else 4  # 默认reduction=4
                act = args[3] if len(args) > 3 else True  # 默认act=True
                args = [c1, c2, kernel_sizes, reduction, act]
            else:
                args = [c1, c2]  # 使用默认值
        elif m is DualStreamPolarityFusion or m is PolarityFusion:
            # 双流极性融合：将RGB图像分离为正负极性分支进行处理
            c1 = ch[f] if isinstance(f, int) else ch[f[0]] if isinstance(f, (list, tuple)) and len(f) > 0 else ch[f]
            c2 = args[0] if len(args) > 0 else c1
            if c2 != nc:
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            # PolarityFusion参数: [c2, k, s, p, g, d, act]
            # 但实际构造函数是: (c1, c2, k, s, p, g, d, act)
            if len(args) >= 1:
                args = [c1, c2, *args[1:]]  # 插入c1作为第一个参数
            else:
                args = [c1, c2]
        elif m is FrequencyYOLOBlock or m is FFTBlock:
            # 频域YOLO块：频域与空域双路径学习
            c1 = ch[f] if isinstance(f, int) else ch[f[0]] if isinstance(f, (list, tuple)) and len(f) > 0 else ch[f]
            c2 = args[0] if len(args) > 0 else c1
            if c2 != nc:
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            # FFTBlock参数: [c2, use_complex]
            # 但实际构造函数是: (c1, c2, use_complex)
            if len(args) >= 1:
                args = [c1, c2, *args[1:]]  # 插入c1作为第一个参数
            else:
                args = [c1, c2]
        elif m is LateralInhibitionBlock or m is LIM or m is LateralInhibition:
            # 侧向抑制块：仿生侧向抑制机制
            c1 = ch[f] if isinstance(f, int) else ch[f[0]] if isinstance(f, (list, tuple)) and len(f) > 0 else ch[f]
            c2 = args[0] if len(args) > 0 else c1
            if c2 != nc:
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            # LIM参数: [c2, k, s, p, g, d, act]
            # 但实际构造函数是: (c1, c2, k, s, p, g, d, act)
            if len(args) >= 1:
                args = [c1, c2, *args[1:]]  # 插入c1作为第一个参数
            else:
                args = [c1, c2]
        elif m is GraphYOLOBlock:
            # 图增强YOLO块：基于图表示的几何推理
            c1 = ch[f] if isinstance(f, int) else ch[f[0]] if isinstance(f, (list, tuple)) and len(f) > 0 else ch[f]
            c2 = args[0] if len(args) > 0 else c1
            if c2 != nc:
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            # GraphYOLOBlock构造函数: (c1, c2, k, s, p, g, d, act, num_nodes)
            # YAML参数: [c2, k, s, p, g, d, act, num_nodes]
            if len(args) >= 1:
                # 提取所有参数
                k = args[1] if len(args) > 1 else 3
                s = args[2] if len(args) > 2 else 1
                p = args[3] if len(args) > 3 else None
                g = args[4] if len(args) > 4 else 1
                d = args[5] if len(args) > 5 else 1
                act = args[6] if len(args) > 6 else True
                num_nodes = args[7] if len(args) > 7 else 64
                args = [c1, c2, k, s, p, g, d, act, num_nodes]
            else:
                args = [c1, c2]  # 使用默认值
        elif m is GCNBlock:
            # GCNBlock工厂函数：需要特殊处理，因为num_nodes通过kwargs传递
            c1 = ch[f] if isinstance(f, int) else ch[f[0]] if isinstance(f, (list, tuple)) and len(f) > 0 else ch[f]
            c2 = args[0] if len(args) > 0 else c1
            if c2 != nc:
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            # GCNBlock参数: [c2, k, s, p, g, d, act, num_nodes]
            # GCNBlock构造函数: (c1, c2, k, s, p, g, d, act, **kwargs) where num_nodes in kwargs
            if len(args) >= 1:
                # 提取所有参数，num_nodes需要单独处理
                k = args[1] if len(args) > 1 else 3
                s = args[2] if len(args) > 2 else 1
                p = args[3] if len(args) > 3 else None
                g = args[4] if len(args) > 4 else 1
                ...
                act = args[6] if len(args) > 6 else True
                num_nodes = args[7] if len(args) > 7 else 64
                # GCNBlock只接受位置参数到act，num_nodes通过kwargs传递
                # 但parse_model不支持kwargs，所以我们需要直接调用GraphYOLOBlock
                # 或者修改GCNBlock使其接受num_nodes作为位置参数
                args = [c1, c2, k, s, p, g, d, act, num_nodes]
            else:
                args = [c1, c2]  # 使用默认值
        elif m is ConnectivityBlock or m is ConnectivityAwareBlock:
            # ConnectivityAwareBlock: 结构连通性感知模块，用于事件图像的轮廓形成
            # 参数格式: [c2, k, s, num_nodes, k_neighbors, message_passing_layers]
            c1 = ch[f] if isinstance(f, int) else ch[f[0]] if isinstance(f, (list, tuple)) and len(f) > 0 else ch[f]
            c2 = args[0] if len(args) > 0 else c1
            if c2 != nc:
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            if len(args) >= 1:
                k = args[1] if len(args) > 1 else 3
                s = args[2] if len(args) > 2 else 1
                num_nodes = args[3] if len(args) > 3 else 128
                k_neighbors = args[4] if len(args) > 4 else 8
                message_passing_layers = args[5] if len(args) > 5 else 2
                args = [c1, c2, k, s, num_nodes, k_neighbors, message_passing_layers]
            else:
                args = [c1, c2]  # 使用默认值
        else:
            c2 = ch[f]
