...
def bbox_iou(
    ...
    Args:
        ...
        ShapeIoU (bool, optional): If True, calculate Shape-IoU with shape size prior.
        eps (float, optional): A small value to avoid division by zero.
    ...
    iou = inter / union
    if ShapeIoU:
        # Shape-IoU: More Accurate Metric with Shape Size Prior
        # Reference: Zhang, H., et al. "Shape-IoU: More Accurate Metric with Shape Size Prior 
        # for Instance Bounding Box Regression." 2023/2024
        # Calculate shape distance penalty based on aspect ratio and size difference
        ww = 2 * (w2 - w1).abs() / (w2 + w1 + eps)  # width difference ratio
        hh = 2 * (h2 - h1).abs() / (h2 + h1 + eps)  # height difference ratio
        # Aspect ratio penalty
        v = (4 / math.pi**2) * ((w2 / h2).atan() - (w1 / h1).atan()).pow(2)
        # Shape distance penalty (combines size and aspect ratio differences)
        with torch.no_grad():
            alpha = v / (v - iou + (1 + eps))
        # Shape-IoU = IoU - shape_penalty
        shape_penalty = (ww.pow(2) + hh.pow(2)) * 0.5 + v * alpha
        return iou - shape_penalty
